...
<body>
	...
	<form action="/NCU_MIS_SA/api/booking.do" method="post">
		<label for="date">日期</label> <select name="date"
			id="date">
			<%
			for (String date: movie.getDatesString()) {
			%>
			<option value="<%=date%>">
				<%=date%>
			</option>
		...
		<label for="date">日期</label> <input type="date" id="date" name="date">
		<label for="showing-time">場次</label> <select name="showing-time"
			id="showing-time">
			<%
